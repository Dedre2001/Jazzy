...
基于上述背景，本研究提出基于多源光谱融合的水稻种质资源抗旱性快速评价方法。本研究旨在系统揭示多光谱反射率、静态荧光和OJIP荧光动力学三种模态对干旱胁迫的响应特征及其互补性，阐明“结构-功能”多维光谱信息的协同机制。在方法创新方面，本研究率先将2025年最新的表格数据基础模型（TabPFN）引入作物表型预测领域，构建“TabPFN预测+TreeSHAP解释”的分析框架，致力于解决小样本农学数据建模难、黑箱模型解释难的问题，为大规模种质资源筛选提供高效、可解释的技术支撑。
...
**形态学评价指标**是植物适应干旱最直观的表现。Uga et al. (2013) 发现了控制水稻根系构型的主效基因 *DEEPER ROOTING 1*，证实了深根系比例和根冠比是水稻抗旱性的核心形态基础，发达的根系能显著提升植株对深层土壤水分的获取能力。地上部性状方面，IRRI (2013) 发布了标准评价手册，采用1-9级视觉评分法对抗旱过程中的叶片卷曲度进行定性评价。然而，Wasaya et al. (2018) 综述了干旱表型分析中的根系研究技术，指出传统的“挖掘-清洗”方法不仅具有破坏性，且难以保证根系的完整性，导致测量误差较大。

为了揭示耐旱的内在机制，研究者开发了一系列**生理生化评价指标**。Barrs & Weatherley (1962) 建立了测定叶片相对含水量（RWC）的标准协议，该指标至今仍被视为衡量植物水分亏缺程度的金标准。在渗透调节方面，Szabados & Savouré (2010) 阐明了脯氨酸（Proline）作为多功能氨基酸的生理作用，证实干旱胁迫会激活 P5CS 酶途径并抑制 P5CDH 酶途径，导致脯氨酸大量积累以维持细胞膨压并清除活性氧（ROS）。Dhindsa et al. (1981) 揭示了叶片衰老与膜通透性及脂质过氧化之间的关联，提出丙二醛（MDA）含量的升高可作为细胞膜系统受损的可靠指标。尽管这些指标具有明确的生物学意义，但测定过程繁琐，单一技术人员每天仅能处理极少量样本，且无法实现同一株植物的连续监测。

**产量与综合评价指数**是抗旱性评价的终极目标。Fischer & Maurer (1978) 提出了干旱敏感性指数（DSI），通过量化干旱下产量的下降幅度来评估品种的稳定性。Fernandez (1992) 进一步建立了抗旱性指数（STI），利用胁迫与正常环境下产量的乘积作为评价指标，倾向于筛选出“高产且抗逆”的基因型。然而，基于产量的评价周期通常长达120天以上，且极易受光温、病虫害等环境噪声的干扰，导致年份间结果的重现性欠佳。
...
进入21世纪，传感器技术向着高通量、高空间分辨率方向快速发展。Feng et al. (2024) 采用多传感器数据融合技术对番茄干旱胁迫进行识别，利用集成在无人机（UAV）上的多光谱相机获取冠层特征，通过结合RGB、多光谱和热红外特征，在干旱状态分类中达到了90%以上的准确率。Zhang et al. (2024) 开展了基于全基因组关联分析（GWAS）的水稻抗旱性研究，利用高通量表型平台获取的大规模光谱数据作为表型输入，成功定位了多个控制水稻抗旱性的关键基因。
...
表格数据基础模型（Tabular Foundation Models）的突破为这一难题提供了新解。Hollmann et al. (2023) 提出了 TabPFN 模型，该模型基于 Transformer 架构在数百万个合成数据集上进行离线预训练，通过学习贝叶斯推理的先验模式，实现了在全新小样本任务上的零样本（Zero-shot）推理。Badaro et al. (2023) 综述了表格数据 Transformer 的应用，指出此类模型在处理非均匀、非线性特征交互方面具有天然优势。Sabo et al. (2025) 率先将 TabPFN 应用于次国家级规模的作物产量预测，结果证实该模型在无需超参数调优的情况下，其预测精度显著超越了经典的随机森林和梯度提升模型。

可解释人工智能（XAI）则是消除农业黑箱模型信任危机的关键。Lundberg et al. (2020) 将基于博弈论的 SHAP 算法应用于树模型解释，实现了特征贡献的局部与全局量化。该方法目前已被广泛应用于解析环境因子（如降水、施肥）对产量的非线性影响，但在多源光谱协同机理的挖掘方面仍有待深入。

### 1.2.5 现有研究的不足与发展趋势

尽管光谱技术与 AI 算法各自取得了长足进展，但聚焦于水稻抗旱性快速评价的系统性研究仍存在明显空白。首先，现有研究多关注单一的光谱或荧光模态，缺乏对 OJIP 光合动力学与多光谱反射、静态荧光的系统性整合研究，“结构-功能”信息的互补机制尚不明确。其次，在样本量有限的科研场景下，如何应用 TabPFN 等基础模型提升表型预测的稳健性尚未得到验证。最后，多数研究止步于“黑箱预测”，未能利用 SHAP 交互值等前沿技术深入挖掘多源数据产生协同增益的生物学本质。

基于此，本研究拟构建基于多源光谱融合的水稻抗旱性评价体系，通过引入 TabPFN 模型解决小样本建模难题，并结合 SHAP 交互分析揭示“光合-代谢”协同响应干旱的科学机理，力求在方法论和理论机制层面实现双重突破。
